import { FaKey, FaUserTie, FaPhone } from "react-icons/fa";
                                ...
                                <div key={name} className="mb-4 relative">
                                    ...
                                    {formik.touched[name as keyof typeof formik.touched] &&
                                        formik.errors[name as keyof typeof formik.errors] && (
                                            <p className="text-red-500 text-sm mt-1">{formik.errors[name as keyof typeof formik.errors]}</p>
                                        )}
                                </div>
                            ))}
                            <button type="submit" className="w-full bg-gray-800 text-white p-2 rounded-md hover:bg-gray-600 cursor-pointer">
                                Sign Up
                            </button>
                        </form>
                        <p className="mt-4 text-center text-black">
                            Have an account? <Link to="/auth/login" className="text-indigo-600">Login</Link>
                        </p>
                    </div>
                </div>
            </div>
